...
class PDFService {
  ...
  addFooter(pdf, pageNum) {
    pdf
      .moveTo(50, pdf.page.height - 70)
      .lineTo(550, pdf.page.height - 70)
      .stroke();
    pdf.fontSize(8);
    pdf.text('MyNet.tn - نظام إدارة المناقصات والمشتريات', 50, pdf.page.height - 60, {
      align: 'center',
    });
    pdf.text(`صفحة ${pageNum}`, pdf.page.width / 2, pdf.page.height - 50, { align: 'center' });
    pdf.text(
      `تاريخ التوليد: ${new Date().toLocaleString('ar-TN')}`,
      pdf.page.width / 2,
      pdf.page.height - 40,
      { align: 'center' }
    );
    pdf.text('وثيقة سرية - قابلة للتدقيق والتحقق', pdf.page.width / 2, pdf.page.height - 30, {
      align: 'center',
    });
  }
  ...
  async generateTenderDocument(tenderId, isDraft = false) {
    ...
    try {
      ...
      pdf.moveDown(0.3);
      pdf.fontSize(10).font('Helvetica');
      pdf.text(`النشر: ${new Date(tender.publish_date).toLocaleDateString('ar-TN')}`, {
        align: 'right',
      });
      pdf.text(`الإغلاق: ${new Date(tender.deadline).toLocaleDateString('ar-TN')}`, {
        align: 'right',
      });
      pdf.text(`الفتح: ${new Date(tender.opening_date).toLocaleDateString('ar-TN')}`, {
        align: 'right',
      });
      pdf.moveDown(1);
      ...
      pdf
        .fontSize(8)
        .text('امسح هذا الرمز للتحقق من صحة الوثيقة', pdf.page.width - 150, pdf.y + 85, {
          width: 100,
          align: 'center',
        });
  ...
  async generateOfferEvaluationReport(offerId, isDraft = false) {
    ...
    try {
      ...
      pdf
        .fontSize(8)
        .text('امسح هنا للتحقق', pdf.page.width - 150, pdf.y + 110, {
          width: 100,
          align: 'center',
        });
  ...
  async generateAwardCertificate(tenderId, supplierId) {
    ...
    try {
      ...
      pdf
        .fontSize(11)
        .text(`تاريخ الترسية: ${new Date().toLocaleDateString('ar-TN')}`, { align: 'center' });
      pdf.moveDown(2);
  ...
  async generateTransactionReport(supplierId, startDate, endDate) {
    ...
    try {
      ...
      if (offersResult.rows.length > 0) {
        ...
        this.createTable(pdf, tableData, ['رقم العرض', 'المناقصة', 'المبلغ', 'الحالة', 'التاريخ'], {
          width: 500,
          rowHeight: 20,
        });
      }
      ...
      pdf
        .fontSize(9)
        .text(`تم إنشاء التقرير: ${new Date().toLocaleString('ar-TN')}`, { align: 'center' });
