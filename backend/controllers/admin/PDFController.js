...
class PDFController {
    ...
    async generateAwardCertificate(req, res) {
        try {
            ...
            res.setHeader('Content-Type', 'application/pdf');
            res.setHeader('Content-Disposition', `attachment; filename="award_certificate_${tender_id}_${supplier_id}.pdf"`);
            res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
    ...
    async generateTransactionReport(req, res) {
        try {
            ...
            res.setHeader('Content-Type', 'application/pdf');
            res.setHeader('Content-Disposition', `attachment; filename="transactions_${supplier_id}.pdf"`);
            res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
