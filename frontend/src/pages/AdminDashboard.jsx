...
import {
  ...
  Grid, Alert, Chip, Avatar, Card, CardContent
} from '@mui/material';
import {
  Dashboard, People, BarChart, Person, Settings, HealthAndSafety, Security,
  Assessment, CheckCircle, TrendingUp, ArrowUpward, ArrowDownward, Database,
  Cloud, Lock
...
function AdminDashboardContent() {
  ...
  const SystemStatusItem = ({ title, status }) => (
    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 2, borderBottom: '1px solid #f0f0f0', '&:last-child': { borderBottom: 'none' } }}>
    ...
    if (activeSection === 'dashboard') {
      return (
        <Box>
          {/* Header */}
          <Box sx={{ mb: 4 }}>
            <Typography variant="h4" sx={{ fontWeight: 700, color: '#000', mb: 1 }}>لوحة تحكم الإدارة</Typography>
            <Typography variant="body2" sx={{ color: '#666' }}>مراقبة وإدارة النظام والمستخدمين</Typography>
          </Box>

          {/* Stats Grid */}
          <Grid container spacing={3} sx={{ mb: 4 }}>
            <Grid item xs={12} sm={6} md={3}>
              <StatCard title="إجمالي المستخدمين" value={stats.totalUsers} change={12} icon={People} color="#0056B3" />
            </Grid>
            <Grid item xs={12} sm={6} md={3}>
              <StatCard title="إجمالي المناقصات" value={stats.totalTenders} change={8} icon={Assessment} color="#2e7d32" />
            </Grid>
            <Grid item xs={12} sm={6} md={3}>
              <StatCard title="إجمالي العروض" value={stats.totalOffers} change={24} icon={TrendingUp} color="#f57c00" />
            </Grid>
            <Grid item xs={12} sm={6} md={3}>
              <StatCard title="حالة النظام" value={stats.systemHealth === 'good' ? 'جيدة' : 'تحذير'} change={0} icon={HealthAndSafety} color="#0288d1" />
            </Grid>
          </Grid>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              <Paper sx={{ p: 3, borderRadius: '12px', border: '1px solid #e0e0e0' }}>
                ...
                <Divider sx={{ mb: 2 }} />
                <Stack spacing={2}>
                  <Alert severity="info" sx={{ mb: 0 }}>10 مستخدمين جدد أمس</Alert>
                  <Alert severity="success" sx={{ mb: 0 }}>5 مناقصات نشطة جديدة</Alert>
                  <Alert severity="warning" sx={{ mb: 0 }}>20 عرض قيد المراجعة</Alert>
                </Stack>
              </Paper>
            </Grid>
          </Grid>
          ...
          <Paper sx={{ p: 3, borderRadius: '12px', border: '1px solid #e0e0e0', mt: 3 }}>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6} md={3}>
                <Button fullWidth variant="outlined" startIcon={<People />} onClick={() => navigate('/admin/users')}>
                  إدارة المستخدمين
                </Button>
              </Grid>
              <Grid item xs={12} sm={6} md={3}>
  ...
  const drawerContent = (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Box sx={{ p: 3, borderBottom: '1px solid #e0e0e0', background: 'linear-gradient(135deg, #D32F2F 0%, #B71C1C 100%)', color: 'white' }}>
        <Typography variant="h6" sx={{ fontWeight: 700 }}>⚙️ الإدارة</Typography>
      </Box>
      <List sx={{ flex: 1, pt: 0 }}>
        {menuItems.map((item) => (
          <ListItemButton
