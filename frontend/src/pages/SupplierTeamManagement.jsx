...
import supplierTeamManagementApi from '../api/supplierTeamManagementApi';
import institutionalTheme from '../theme/theme';
import LoadingSpinner from '../components/LoadingSpinner';

export default function SupplierTeamManagement() {
  ...
  useEffect(() => {
    setPageTitle('إدارة فريق المزود');
    fetchData();
  }, []);
  ...
  const fetchData = async () => {
    try {
      ...
      const [membersRes, statsRes] = await Promise.all([
        supplierTeamManagementApi.getTeamMembers(),
        supplierTeamManagementApi.getTeamStats(),
      ]);
  ...
  const handleSave = async () => {
    try {
      if (editingMember) {
        await supplierTeamManagementApi.updateTeamMember(editingMember.id, formData);
        setMessage({ type: 'success', text: 'تم تحديث العضو بنجاح' });
      } else {
        await supplierTeamManagementApi.addTeamMember(formData);
  ...
  const handleDelete = async (memberId) => {
    if (window.confirm('هل أنت متأكد من حذف هذا العضو؟')) {
      try {
        await supplierTeamManagementApi.removeTeamMember(memberId);
